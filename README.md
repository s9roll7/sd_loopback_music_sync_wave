...
- Set [Allow other script to control this extension](https://github.com/s9roll7/ebsynth_utility#note-2--how-to-use-multi-controlnet-together) True.
- Configure the controlnet itself as you normally use it, set it to enable, then configure the preprocessor and mode settings  
- Create the initial image. Create a normal img2img image of the very first frame of the video and [Send to Img2Img]. You should set denoising strength to 0.7 or a higher value. Also, generate the image with the control net enabled. The first frame can be extract by following the steps.  
![first_frame](imgs/first_frame.png "extract first frame")  

- Specify the same path in [Project Directory(optional)] and [Video File Path(optional)] as specified in the frame creation procedure above.  
...
- During video generation, a controlnet cache is generated in project_dir to speed up the process. Therefore, the second time the video is generated, the generation time will be shorter. If you want to change the input videos, please change the project_dir as well, since it will be inconsistent with the cache.  

<br>

### loopback + controlnet + optical flow (sample 5)
- Follow the steps in "loopback + controlnet (sample 2)" until just before Generate.  
- Make the following settings
- If [Interpolation Multiplier] is set, interpolated frames are generated at the end of video generation.
- Generate  
![optical_flow](imgs/optical_flow.png "optical flow")  
- A cache about flow is also generated in project_dir. Note that the cache is created for each [final fps = Frames per second x interpolation multiplier]. In other words, if you generate 24 in [Frames per second] and 1 in [interpolation multiplier], and then generate 8 in [Frames per second] and 3 in [interpolation multiplier], the cache has already been generated, so This will shorten the generation time.
